    ...
    <article key={page.href}>
      ...
      >
        ...
        <Link
          ...
          aria-label={`Explorer le sujet: ${page.title}`}
        >
          Explorer le sujet
...
export function PageLinks({ title, pages, intro, className, category }) {
  // category: string | string[] | undefined
  const matchesCategory = (p) => {
    if (!category) return true
    if (Array.isArray(category)) return category.some(c => p.category === c)
    return p.category === category
  }

  const filtered = pages.filter(matchesCategory)
